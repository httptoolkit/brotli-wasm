...
describe('Brotli-wasm', () => {
    ...
    for (const bundlerToUse of [bundlerName, webName]) {
        ...
        describe(bundlerTestName, function () {
            ...
            it('can compress data', async () => {
                const input = Buffer.from('Test input data');
                const result = brotli.compress(input);
                expect(Buffer.from(result).toString('base64')).to.equal(
                    'Gw4A+KWpyubolCCjVAjmxJ4D'
                );
            });
            ...
            it('can decompress data', () => {
                ...
                const input = Buffer.from(
                    'GxoAABypU587dC0k9ianQOgqjS32iUTcCA==',
                    'base64'
                );
                const result = brotli.decompress(input);
                expect(Buffer.from(result).toString('utf8')).to.equal(
            ...
            it('cleanly fails when decompressing garbage', () => {
                const input = Buffer.from(
                    "This is not brotli data, it's just a string"
                );
                expect(() => brotli.decompress(input)).to.throw(
            ...
            it('can compress & decompress back to the original result with a different quality setting', () => {
                ...
                const result = Buffer.from(
                    brotli.decompress(
                        brotli.compress(Buffer.from(input), { quality: 3 })
                    )
                ).toString('utf8');
                expect(result).to.equal(input);
            });
            ...
            it('can streamingly compress data', () => {
                ...
                expect(stream.result()).to.equal(
                    brotli.BrotliStreamResult.NeedsMoreInput
                );
                ...
                expect(stream.result()).to.equal(
                    brotli.BrotliStreamResult.ResultSuccess
                );
                expect(
                    Buffer.concat([output1, output2, output3]).toString(
                        'base64'
                    )
                ).to.equal('Gw4A+KWpyubolCCjVAjmxJ4D');
            });
            ...
            it('can streamingly compress data with a different quality setting', () => {
                ...
                expect(
                    Buffer.from(brotli.decompress(output)).toString('base64')
                ).to.equal(input.toString('base64'));
            });
            ...
            it('can streamingly decompress data', () => {
                ...
                expect(
                    Buffer.concat([output1, output2]).toString('utf8')
                ).to.equal('Brotli brotli brotli brotli');
            });
            ...
            it('cleanly fails when streamingly decompressing garbage', () => {
                ...
                const stream = new brotli.DecompressStream();
                expect(() => stream.decompress(input, 100)).to.throw(
            ...
            it('can streamingly compress & decompress back to the original result', () => {
                ...
                expect(encStream.result()).to.equal(
                    brotli.BrotliStreamResult.NeedsMoreInput
                );
                ...
                expect(decStream.result()).to.equal(
                    brotli.BrotliStreamResult.NeedsMoreInput
                );
            ...
            it('streaming compressing can handle needing more output when action is process', function () {
                ...
                const output2 = stream.compress(
                    input.slice(stream.last_input_offset()),
                    1500000
                );
                expect(stream.result()).to.equal(
                    brotli.BrotliStreamResult.NeedsMoreInput
            ...
            it('streaming compressing can handle needing more output when action is finish', () => {
                const input = Buffer.from(
                    'Some thrilling text I urgently need to compress'
                );
                const stream = new brotli.CompressStream();
                const output1 = stream.compress(input, 1);
                expect(stream.result()).to.equal(
            ...
            it('streaming decompressing can handle needing more output', () => {
                ...
                const output2 = stream.decompress(
                    input.slice(stream.last_input_offset()),
                    100
                );
                expect(stream.result()).to.equal(
