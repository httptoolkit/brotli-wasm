...
pub fn GetCopyLengthCode(copylen: usize) -> u16 {
    // Brotli copy codes start at length 2 (code 0 encodes length 2). A copylen of 0 or 1
    // violates the encoder invariant; in debug fail loudly right here instead of wrapping to
    // 65534/65535 and trapping later with an unhelpful index-out-of-bounds. In release,
    // saturate to code 0 so a latent bug degrades to a wrong-but-parseable stream rather than
    // a far-away OOB panic. The fix_unbroken_len guard in backward_references ensures the
    // valid case (copylen >= 2) always holds, so this is purely defensive.
    debug_assert!(copylen >= 2, "brotli copy length {} is below the minimum of 2", copylen);
    if copylen < 10usize {
        copylen.saturating_sub(2) as u16
