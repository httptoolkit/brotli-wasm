...
/// Minimum copy length representable in a brotli backward reference: copy code 0 encodes
/// a length of 2, so an effective copy length of 0 or 1 is invalid and must never reach
/// the bitstream (it underflows `GetCopyLengthCode`'s `copylen - 2`).
const BROTLI_MIN_BACKWARD_COPY_LEN: usize = 2;
...
///
/// Returns 0 when the dictionary break would truncate the match below
/// [`BROTLI_MIN_BACKWARD_COPY_LEN`]. Such a candidate is not a valid brotli copy and callers
/// treat 0 as "no usable match" (the bytes are emitted as literals instead). This is the root
/// cause of the custom-dictionary encoder panic at qualities 5-9: without this guard, a match
/// straddling the custom-dictionary boundary could be truncated to length 1 (or 0) and emitted
/// as an invalid backward reference whose copy length code underflows to 65535.
fn fix_unbroken_len(
    unbroken_len: usize,
    prev_ix: usize,
    ...
    if let Some(br) = ring_buffer_break {
        if prev_ix < usize::from(br) && prev_ix + unbroken_len > usize::from(br) {
            let truncated = usize::from(br) - prev_ix;
            if truncated < BROTLI_MIN_BACKWARD_COPY_LEN {
                return 0;
            }
            return truncated;
        }
        ...
        if prev_ix < cur_ix {
            ...
            if compare_char == data[prev_ix.wrapping_add(best_len)] as i32 {
                ...
                if unbroken_len != 0 {
                    ...
                    // A custom-dictionary match straddling the dictionary boundary can be
                    // truncated below the minimum valid copy length (fix_unbroken_len => 0);
                    // such a candidate must not be emitted as a backward reference.
                    if len >= BROTLI_MIN_BACKWARD_COPY_LEN {
